 ...
 */
BTUUID::BTUUID(std::string value) {
	m_valueSet = true;
	if (value.length() == 2) {
 ...
 */
BTUUID::BTUUID(uint8_t* pData, size_t size, bool msbFirst) {
	if (size != 16) {
 ...
 */
BTUUID::BTUUID(esp_bt_uuid_t uuid) {
	m_uuid     = uuid;
	m_valueSet = true;
} // BTUUID
 ...
 */
BTUUID::BTUUID(esp_gatt_id_t gattId) : BTUUID(gattId.uuid) {
} // BTUUID
 ...
 */
bool BTUUID::equals(BTUUID uuid) {
...
/**
 * Create a BTUUID from a string of the form:
 * 0xNNNN
 * 0xNNNNNNNN
 * 0x<UUID>
 * NNNN
 ...
 * <UUID>
 */
BTUUID BTUUID::fromString(std::string _uuid){
	uint8_t start = 0;
 ...
 */
esp_bt_uuid_t* BTUUID::getNative() {
